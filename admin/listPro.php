...
//得到数据库中的所有商品
$sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from imooc_pro as p join imooc_cate as c on p.cId = c.id {$where}";
...
$sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from imooc_pro as p join imooc_cate as c on p.cId = c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($sql);
if(!$rows){
...
<div class="details">
    ...
    <table class="table" cellspacing="0" cellpadding="0">
                	...
                	<table class="table" cellspacing="0" cellpadding="0">
                		...
                		<tr>
                			...
                			<td>
                                ...
                                ?>
                                <img width="100" height="100" src="uploads/<?php echo $proImg['albumPath'];?>" alt="" />&nbsp;&nbsp;
                                <?php endforeach;?>
                			</td>
                		</tr>
                		<tr>
        ...
        <?php if ($totalRows > $pageSize):?>
			<tr>
			     <td colspan="7" align="center"><?php echo showPage($page, $totalPage,"keywords={$keywords}&$order={$order}");?></td>
			</tr>
		<?php endif;?>
        </tbody>
    </table>
...
function showDetail(id,t){
	...
	function delPro(id){
		if(window.confirm("您确定要删除吗？添加一次不易，且删且珍惜！")){
